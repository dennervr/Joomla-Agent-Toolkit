...
def test_search_docs_success():
    ...
    mock_results = [
        (
            MagicMock(
                ...
                metadata={"source": "file1.md", "Header 1": "Topic 1"},
            ),
            0.9,
        ),
    ...
    with patch(
        "joomla_rag.search.HuggingFaceEmbeddings", return_value=mock_embeddings
    ) as mock_embeddings_class, patch(
        "joomla_rag.search.Chroma", return_value=mock_vectorstore
    ) as mock_chroma_class, patch(
...
def test_search_docs_no_results():
    ...
    with patch(
        "joomla_rag.search.HuggingFaceEmbeddings", return_value=mock_embeddings
    ), patch("joomla_rag.search.Chroma", return_value=mock_vectorstore), patch(
        "pathlib.Path.exists", return_value=True
    ), patch(
        "builtins.print"
    ) as mock_print:
