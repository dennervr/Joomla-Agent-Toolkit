...
try:
    ...
    print(
        "Error: RAG dependencies are not installed. Run: pip install langchain-community langchain-huggingface chromadb sentence-transformers",
        file=sys.stderr,
    )
    sys.exit(1)
...
def search_docs(query: str, k: int = 5):
    ...
    if not chroma_persist_dir.exists():
        print(
            f"Error: Packaged vector database not found in {chroma_persist_dir}.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    try:
        embeddings = HuggingFaceEmbeddings(
            model_name="all-MiniLM-L6-v2", model_kwargs={"local_files_only": True}
        )
